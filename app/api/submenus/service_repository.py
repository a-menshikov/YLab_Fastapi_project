...
class SubmenuService:
    ...
    async def get_all_submenus(self, background_tasks: BackgroundTasks,
                               menu_id: str) -> list[Submenu]:
        ...
        background_tasks.add_task(self.cache_repo.set_all_submenus_cache,
                                  menu_id=menu_id,
                                  items=items)
        return items

    async def get_submenu_by_id(self, background_tasks: BackgroundTasks,
                                id: str, menu_id: str) -> Submenu:
        ...
        background_tasks.add_task(self.cache_repo.set_submenu_cache, item)
        return item

    async def create_submenu(self, background_tasks: BackgroundTasks,
                             submenu: SubmenuPost,
                             menu_id: str) -> Submenu:
        """Добавление нового подменю."""
        item = await self.crud_repo.create_submenu(submenu=submenu,
                                                   menu_id=menu_id)
        background_tasks.add_task(self.cache_repo.create_submenu_cache, item)
        return item

    async def update_submenu(self, background_tasks: BackgroundTasks,
                             submenu_id: str,
        ...
        background_tasks.add_task(self.cache_repo.update_submenu_cache, item)
        return item

    async def delete_submenu(self, background_tasks: BackgroundTasks,
                             menu_id: str, submenu_id: str) -> None:
        """Удаление подменю конкретного меню по id."""
        background_tasks.add_task(self.cache_repo.delete_submenu_cache,
                                  submenu_id=submenu_id,
                                  menu_id=menu_id)
